    ...
    if (isset($_GET['update'])) {
        ...
        if (isset($_POST['submit'])) {
            ...
            $update_query = "UPDATE `17.2.2` SET `sdg` = '$sdg', `title` = '$title', `description` = '$description', `cost` = '$cost', `source` = '$source', `pts` = $pts  WHERE `id` = '$id'";

            $update = mysqli_query($conn, $update_query);

            if ($update) {
                header("Location:../edit/edit17.2.2.php?message=Update Successful");
            }
        }
        $sql = "SELECT * FROM `17.2.2` WHERE id = '$id'";
    ...
    <div class="collapse w-100 h-auto p-3" style="display: block ;">
        <h2 class="text-center pb-5">Cross sectoral dialogue about SDGs</h2>
        <form action="" method="post">
            <div class="input-group mb-3">
                <span class="input-group-text" id="basic-addon1">Total number of cross-sectoral SDG dialogue initiated and/or
participated</span>
