    ...
    if (isset($_GET['update'])) {
        ...
        if (isset($_POST['submit'])) {
            ...
            $update_query = "UPDATE `17.2.3` SET `sdg` = '$sdg', `title` = '$title', `description` = '$description', `cost` = '$cost', `source` = '$source', `pts` = $pts  WHERE `id` = '$id'";

            $update = mysqli_query($conn, $update_query);

            if ($update) {
                header("Location:../edit/edit17.2.3.php?message=Update Successful");
            }
        }
        $sql = "SELECT * FROM `17.2.3` WHERE id = '$id'";
    ...
    <div class="collapse w-100 h-auto p-3" style="display: block ;">
        <h2 class="text-center pb-5">International collaboration data gathering for SDG</h2>
